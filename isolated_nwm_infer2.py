...
def main(args):
    ...
    if not args.gt:
        ...
        if weight_key not in ckp:
            fallback = 'model' if 'model' in ckp else 'ema'
            print(f"[infer] weight key '{weight_key}' not in checkpoint (keys={list(ckp.keys())}); using '{fallback}'")
            weight_key = fallback
        print(f"[infer] loading weights from key '{weight_key}'")
        print(model.load_state_dict(ckp[weight_key], strict=True))
        model.eval()
        model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight_key", type=str, default='ema', help="checkpoint key to load weights from: 'ema' (default) or 'model' (raw trained weights)")
